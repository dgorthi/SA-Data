"""
Modified plot_uv.py so it can be called like a module in another script.
...
def plot_uv_func(files,mode='log',time='all',unmask=False,delay=False,fringe=False,\
            dt=False,df=False,out_file='',time_axis='index',chan_axis='index',\
            clean=None,nolegend=False,share=False,xlim=None,ylim=None,plot_each=None,\
            window='blackman-harris',src=None,ant=None,pol=None,chan=None,dec=None,\
            cmap='jet',Max=None,drng=None,cal=None,decphs=0,cat='helm,misc',decimate=1):

    """
    Parameters:
    ----------
    mode: default='log', Plot mode can be log (logrithmic), lin (linear), phs (phase), real, or imag. default='log'
    
    time: default='all', Select which time sample to plot. Options are: "all" (default), "<time1 #>_<time2 #>" (a range of times to plot), or "<time1 #>,<time2 #>" (a list of times to plot). If "all" or a range are selected, a 2-d image will be plotted. If a list of times is selected an xy plot will be generated.')

    unmask: Plot masked data, too.
    
    delay: Boolean, Take FFT of frequency axis to go to delay (t) space.
    
    fringe: Boolean, Take FFT of time axis to go to fringe (Hz) space.
    
    dt: Boolean, Remove a linear extrapolation from adjacent times.
    
    df: Boolean, Remove a linear extrapolation from adjacent frequency channels.
    
    out_file: str, If provided, will save the figure to the specified file instead of popping up a window.
    
    time_axis: default='index', Choose time axis to be integration/fringe index (index), or physical coordinates (physical), or if doing xy plot in time-mode, (lst) is also available.  Default is index.
    
    chan_axis: default='index', Choose channel axis to be channel/delay index (index), or physical coordinates (physical).  Default is index.
    
    clean: float, Deconvolve delay-domain data by the "beam response" that results from flagged data.  Specify a tolerance for termination (usually 1e-2 or 1e-3).
    
    nolegend: Boolean, Omit legend in last plot.
    
    share: Boolean, Share plots in a single frame.
    
    xlim: Limits on the x axis (channel/delay) for plotting.
    
    ylim: Limits on the x axis (time/delay-rate) for plotting.
    
    plot_each: Instead of a waterfall plot, plot each of the specified axis (chan,time)
    
    window: default='blackman-harris', Windowing function to use in delay transform.  Default is blackman-harris.  Options are: ' + ', '.join(a.dsp.WINDOW_FUNC.keys())
    
    """

    # Parse command-line options
    cmap = p.get_cmap(cmap)
    if not xlim == None: xlim = map(float, xlim.split('_'))
    if not ylim == None: ylim = map(float, ylim.split('_'))
    
    uv = a.miriad.UV(files[0])
    a.scripting.uv_selector(uv, ant, pol)
    
    chans = a.scripting.parse_chans(chan, uv['nchan'])
    
    is_chan_range, is_time_range = True, True
    if plot_each == 'chan': is_chan_range = False
    elif plot_each == 'time': is_time_range = False
    ...
    if delay:
        if freqs.size == freqs[-1] - freqs[0] + 1:
            ...
            raise ValueError('Channels must be contiguous to do delay transform (chan=%s)' % (chan))
        delays = np.fft.fftfreq(freqs.size, freqs[1]-freqs[0])
        delays = np.fft.fftshift(delays)
    
    time_sel = gen_times(time, uv, time_axis, decimate)
    inttime = uv['inttime'] * decimate
    if not src is None:
        srclist,cutoff,catalogs = a.scripting.parse_srcs(src, cat)
        src = a.cal.get_catalog(cal, srclist, cutoff, catalogs).values()[0]
    ...
    for uvfile in files:
        ...
        if not cal is None:
            aa = a.cal.get_aa(cal, uv['sdf'], uv['sfreq'], uv['nchan'])
        ...
        for (uvw,t,(i,j)),d in uv.all():
            ...
            d = d.take(chans)
            if ant.find('%d_%d' % (j,i)) != -1: d = d.conj() # obey antenna ordering, if possible
            #apply cal phases
            if not cal is None:
                aa.set_jultime(t)
                if not src is None:
            ...
            if delay:
                w = a.dsp.gen_window(d.shape[-1], window=window)
                if unmask:
                ...
                if not clean is None and not np.all(d == 0):
                    d, info = a.deconv.clean(d, ker, tol=clean)
                    d += info['res'] / gain
                d = np.ma.array(d)
                d = np.fft.fftshift(d, axes=0)
            elif unmask: d = d.data
            d.shape = (1,) + d.shape
    ...
    if not src is None:fig.suptitle(src)
    for cnt, bl in enumerate(bls):
        d = np.ma.concatenate(plot_x[bl], axis=0)
        i,j,pol = map(int,bl.split(','))
        if df: d = d[:,:-2]/2 + d[:,2:]/2 - d[:,1:-1]
        if dt: d = d[:-2]/2 + d[2:]/2 - d[1:-1]
        if fringe:
            d = d.filled(0)
            w = a.dsp.gen_window(d.shape[0], window=window); w.shape += (1,)
            ...
            if not clean is None:
                for chan in range(d.shape[1]):
                    ...
                    d[:,chan],info = a.deconv.clean(d[:,chan],ker[:,chan],tol=clean)
        ...
        if is_chan_range and is_time_range:
            ...
            if not Max is None: dmax = Max
            elif dmax is None: dmax = d.max()
            else: dmax = max(dmax,d.max())
            if not drng is None: dmin = dmax - drng
            elif dmin is None: dmin = d.min()
            else: dmin = min(dmin,d.min())
            
            plots[cnt+1] = ax.imshow(d, extent=(c1,c2,t2,t1), origin='upper',
                aspect='auto', interpolation='nearest', 
                vmax=dmax, vmin=dmin, cmap=cmap)
            fig.colorbar(shrink=0.5)
            ax.set_xlabel(xlabel); ax.set_ylabel(ylabel)
            if not xlim == None: ax.set_xlim(*xlim)
            if not ylim == None: ax.set_ylim(ylim[1],ylim[0]) # Reverse b/c otherwise ylim flips origin for unknown reasons
            ...
            for ti,t in enumerate(plot_t):
                ax.plot(plot_chans, d[ti,:], '-', label=label % t)
            ...
            if not ylim == None: ax.set_ylim(*ylim)
            else: ax.set_ylim(dmin,dmax)
            ...
            if chan_axis == 'index': label += '#%d'
            else:
                chans = freqs
                label += '%f GHz'
            for c, chan in enumerate(chans):
                ax.plot(plot_times, d[:,c], '-', label=label % chan)
            if not Max is None: dmax = Max
            elif dmax is None: dmax = d.max()
            else: dmax = max(dmax,d.max())
            if not drng is None: dmin = dmax - drng
        ...
        if not share:
            ...
            ax.set_title(title)
    if not nolegend and (not is_time_range or not is_chan_range): 
        ax.legend(loc='best')
    
#    # Save to a file or pop up a window
#    if out_file != '': fig.savefig(out_file)
#    else:
#        def click(event):
#            print [event.key]
#            if event.key == 'm':
